...
class Score:
    ...
    def print_measure1_query(self,qid,ret,rel,rel_ret):
        print(("Query ID: %d\n"
               ...
               "    Relevant:        %4d\n"
               "    Rel_Retr:        %4d\n"
        ) % (qid,ret,rel,rel_ret), file=sys.stderr, end='')
    ...
    def print_measure1_summary(self,config):
        ...
        writer = open("outputFile.txt", "w")
        writer.write(str(scores))
        print(format.format(*scores), file=sys.stderr, end='')
        if not config.show_interp_prec:
            print(file=sys.stderr)


    def print_measure2_query(self,inter_pts):
        print("Interpolated Precision:", file=sys.stderr)
        for i in range(self.interp_points + 1):
            print("    at %.2f      =  %.3f" % (
    ...
    def print_measure2_summary(self,config):
        ...
        if config.print_flat:
            for i in range(self.interp_points + 1):
                print('%.3f' % self.global_interpolation_points[i], end='', file=sys.stderr)
        else:
            print("Interpolated Precision - Averaged over all queries:", file=sys.stderr)
            for i in range(self.interp_points + 1):
                print("    at %.2f      =  %.3f" % (
                    (float(i) / self.interp_points),
                    self.global_interpolation_points[i]), file=sys.stderr)
        print(file=sys.stderr)
